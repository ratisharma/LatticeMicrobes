...
def main(sc, outputDir, outputIndices, filename, replicates, minTime, maxTime, speciesToBin, outputFileNum, skipTime, sparse, interactive):
...
def filterLatticeTimeSeries(record):
    ...
    global globalReplicates, globalMinTime, globalMaxTime

    # Get the global variables.
    replicates = globalReplicates.value
    minTime = globalMinTime.value
    maxTime = globalMaxTime.value
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print "Usage: output_dir output_indices hdfs_sfile onEvents_pfile [--skip_less_than=<time>] [--not-sparse] species+ [--interactive]"
        quit()
    ...
    if type(outputIndices) != type((0,)):
        ...
        quit()
    filename = sys.argv[3]
    onEventFile = sys.argv[4]
    # replicates = range(int(sys.argv[4]),int(sys.argv[5])+1)
    # minTime = float(sys.argv[6])
    # maxTime = float(sys.argv[7])
    interactive = False
    sparse = True
    skipTime=0.0
    species=[]
    ...
    # Load on-events and carry out spark analysis:
    onEvents = pickle.load(open(onEventFile,'rb'))

    # Set replicates and minTime, maxTime
    outputFileNum = 6
    for i in range(1,2):
        replicates = [onEvents[i][0]]
        events = onEvents[i][1]
        print replicates
        if len(events) > 0:
            for j in range(2,3):
                ...
                maxTime = events[j][1]
                outputFileNum += 1
                print (replicates,minTime,maxTime)
		time.sleep(5)
                ...
                main(sc, outputDir, outputIndices, filename, replicates, minTime, maxTime, species, outputFileNum, skipTime, sparse, interactive)
