...
replicates="1-25";
numCPUs=26;
dataDir=data_Dkp13_Drl13_memThick2/min_ligand;
logsDir=logs_Dkp13_Drl13_memThick2/min_ligand;
...
for i in $is; do
	...
	#filename=cell_rdme_Lmem_gradSteep_t500_${p}_c1b_${c1b}_c2b_${c2b}_c3b_${c3b}_c0a_${c0a}_c4_${c4}_c5_${c5}_c6_${c6}__Dk_${Dk}_Dkp_${Dkp}_Dr_${Dr}_Drl_${Drl}.lm;
        filename=Dkp13_Drl13_min_ligand.lm
	rm -f ../${dataDir}/${filename};
        rm -f ../${logsDir}/${filename}.log;
	#fileSizeBlocks=0; 
	#if [ -e "${dataDir}/${filename}" ];
        #then fileSizeBlocks=(`ls -s ${dataDir}/${filename}`);
	...
	#lmes-submit ../${dataDir}/${filename} -m lm::rdme::NextSubvolumeSolver -r ${replicates} -cr 1 ../${logsDir}/${filename}.2.log batch ${numCPUs} rsharm28 4:00:00;
	#fi;
	done;
	done;
	done;
