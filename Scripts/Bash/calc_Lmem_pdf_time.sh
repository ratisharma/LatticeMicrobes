...
#!/bin/bash
    ...
    replicates="1-50";
    timePoint=100;
    pythonFile=calc_time_pdf.py
    #outputDir=pdf_gradSteep_memThick2.dat;
    outputDir=pdf_c6_4_memThick2/Dkp_5e-15_t${timePoint}.dat;
		...
		matfilename=`printf %05d $((i+1))`.`printf %05d $((j+1))`.`printf %05d $((k+1))`.mat;
    		...
    		if [ ! -e "../matlab/${outputDir}/${matfilename}" ]; then
         	...
         	if [ ${fileSizeBlocks} -gt 16 ]; then
	      	   ...
	      	   python-submit ${pythonFile} ../matlab/${outputDir} ../${dataDir}/${filename} ${species} ${timePoint} ${i} ${j} ${k} ../${logsDir}/calc_time_pdf_${filename}.${species}.log batch rsharm28 06:00:00;
         	else
