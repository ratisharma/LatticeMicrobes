#!/bin/bash
    ...
    outputDir=pdf_mono_gradSteep_Dkp15/ligand.dat;
    dataDir=data_mono_gradSteep_Dkp15;
    logsDir=logs_mono_gradSteep_Dkp15;
	...
	for j in $js; do
	    for k in $ks; do
	    	...
	    	Drl=${Drls[j]};
	    	species=${k}; 
		filename=cell_rdme_mono_ligand.lm		
		...
		matfilename=`printf %05d $((i+1))`.`printf %05d $((j+1))`.`printf %05d $((k+1))`.mat;
    		...
    		if [ ! -e "../matlab/${outputDir}/${matfilename}" ]; then
         	...
         	if [ ${fileSizeBlocks} -gt 16 ]; then
	      	   ...
	      	   python-submit ${pythonFile} ../matlab/${outputDir} ../${dataDir}/${filename} ${species} ${i} ${j} ${k} ../${logsDir}/calc_pdf_${filename}.${species}.log batch rsharm28 3:00:00;
         	else
