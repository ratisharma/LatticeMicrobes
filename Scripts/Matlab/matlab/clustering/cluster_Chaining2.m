...
function [M]= cluster_Chaining2(filename,replicate,ts,cutoffDistance)
...
L=permute(h5read(inputFilename,sprintf('/Simulations/%07d/Lattice/%010d',replicate,ts)),[4:-1:1]);
for x=1:50
    for y=1:50
        for z=1:50
            for p=1:16
...
% SubvolumeCount=sum((L==(3)),4);
% %     	if sum(sum(sum(SubvolumeCount,3),2))>onThreshold:
