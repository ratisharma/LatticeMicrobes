...
function [msd,cm2]=clusterMSD_surface_traj(filename,replicate,cutoffDistance)
...
lattice=permute(hdf5read(filename,sprintf('/Simulations/%07d/Lattice/%010d',replicate,ts)),[4:-1:1]);
M5=M;
Mcounts=zeros(size(M,1),1);
for xsize=1:size(M,1)
    counts=0;
    for p=1:16
        if lattice(M(xsize,1),M(xsize,2),M(xsize,3),p)==3,
            counts = counts+1;
        end
    end
    Mcounts(xsize)=counts;
end
M5=[M5 Mcounts];
M6a=[M6a M5(:,1)'];
M6b=[M6b M5(:,2)'];
...
% outputFilename=sprintf('../analysisData/Dkp15_Drl15/onEventTraj4.mat');
% save(outputFilename,'M5');
% figure
% subplot(2,1,1)
...
scatter(histTheta,histz.*0.05,400,histCounts,'.')
title(['replicate = ', num2str(replicate), ', traj length = ', num2str(size(onevent1(bigevent(1),:),2))])
xlabel('{\theta}'); ylabel('z ({\mu m})')
caxis manual
% caxis([0.0 0.12]);
...
M6=[M6 M6a' M6b' M6c' M6d' M6e'];
outputFilename=sprintf('../analysisData/Dkp15_Drl15/onEventTraj4.mat');
save(outputFilename,'M6');
% figure
% subplot(2,1,1)
