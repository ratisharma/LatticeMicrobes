...
matfile1=sprintf('analysisData/pdf_delt/pdf_feedback_min_ligand_delt_%d.mat',d);
load(matfile1);
...
matfile2=sprintf('analysisData/pdf_delt/pdf_feedback_gradient_delt_%d.mat',d);
...
[Na,p]=hist(angle,10);
% [Nr,q]=hist(ratio,10);
meanRatioHist=zeros(size(p,2),1);
for id=1:size(p,2)
    if (id==1),
        [idx3]=ind2sub(size(M2(:,1)),find(M2(:,1)<=p(id)));
        for a5=1:size(idx3,1)
        meanRatioHist(id)=meanRatioHist(id)+M2(idx3(a5,1),2)./size(idx3,1);
        end
    else
       [idx3]=ind2sub(size(M2(:,1)),find(M2(:,1)>p(id-1) & M2(:,1)<=p(id)));
       for a5=1:size(idx3,1)
        meanRatioHist(id)=meanRatioHist(id)+M2(idx3(a5,1),2)./size(idx3,1);
        end
    end
end
plot(p,meanRatioHist);
hold on;
% figure
% scatter(angle,ratio);
% hold on;
